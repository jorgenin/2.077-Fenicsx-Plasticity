...
qb = dolfinx.fem.Constant(
    mesh, 0.100
)  # kinematic hardening: saturation value [GPa] (includes factor 2/3)
...
Qe = ffcx.element_interface.QuadratureElement(
    mesh.basix_cell().name, (), degree=quad_degree, scheme="default"
)
...
# Output functions (For visualizaton)
Po = dolfinx.fem.Function(
    dolfinx.fem.TensorFunctionSpace(mesh, ("DG", 0)), name="P"
)  # for output
...
ax1.set_title(
    "Rate-independent plasticity: $J_2$, monolithic formulation, 3D", fontsize=12
)
ax1.set_xlabel(
    r"volume-averaged strain $\frac{1}{V}\int n^T E n \, dV$ [-]", fontsize=12
)
ax1.set_ylabel(
    r"volume-averaged stress $\frac{1}{V}\int n^T S n \, dV$ [GPa]", fontsize=12
)
